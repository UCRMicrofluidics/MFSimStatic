 ...
 * Source: genet_path_scheduler.cc												*
 * Original Code Author(s): Kenneth O'Neal										*
 * Original Completion/Release Date: May 14, 2015								*
 *																				*
 * Details: N/A																	*
 *																				*
 * Revision History:															*
...
// Constructor
///////////////////////////////////////////////////////////////////////////////////
GenetPathScheduler::GenetPathScheduler()
{
...
/////////////////////////////////////////////////////////////////////////////////////
void GenetPathScheduler::setAsRand(vector< map<AssayNode*, unsigned> *>::iterator it, vector< map<AssayNode*, unsigned> *> * pop)
{
...
/////////////////////////////////////////////////////////////////////////////////////
map<AssayNode*,  unsigned> * GenetPathScheduler::rand_key(DAG *dag)
{
...
/////////////////////////////////////////////////////////////////////////////////////
vector< map<AssayNode*, unsigned> *> * GenetPathScheduler::initialize_population(DAG * dag, int A)
{
...
/////////////////////////////////////////////////////////////////////////////////////
vector< map<AssayNode*, unsigned> *> * GenetPathScheduler::mutation(DAG * dag)
{
...
// offspring
/////////////////////////////////////////////////////////////////////////////////////
vector< map<AssayNode*, unsigned> *> * GenetPathScheduler::crossover(vector< map<AssayNode*, unsigned> *> * popA, DAG * dag)
{
...
/////////////////////////////////////////////////////////////////////////////////////
vector< map<AssayNode*, unsigned> *> * GenetPathScheduler::reproduction(vector< map<AssayNode*, unsigned> *> * popA, vector<unsigned> * TS )
{
	...
	for( unsigned k = 0; k< rep_size; k++)
	{
		...
		//cout<<"rep_pop size is:"<<rep_pop->size()<<flush<<endl;
		//cout<<"popA size is:"<<popA->size()<<flush<<endl;
...
/////////////////////////////////////////////////////////////////////////////////////
void GenetPathScheduler::Dag_reinitialize(DAG * dag)
{
...
// overs and mutations.
/////////////////////////////////////////////////////////////////////////////////////
unsigned long long GenetPathScheduler::schedule(DmfbArch *arch, DAG *dag)
{
	...
	PathScheduler ps;
	ps.setMaxStoragePerModule(getMaxStoragePerModule());
	ps.setPrioritiesExternally(); // GenetScheduler will set the priorities itself
	...
	//cout<<"init_pop1 size is: "<<init_pop->size()<<flush<<endl;
	...
	while(count < num_generations)
	{
		...
		{
			//cout<<"before set PS"<<flush<<endl;
			PathScheduler ps;
			ps.setMaxStoragePerModule(getMaxStoragePerModule());
			ps.setPrioritiesExternally();

			//cout<<"before reinit"<<flush<<endl;
			//reinitialize dag
			Dag_reinitialize(dag);
			map<AssayNode*, unsigned> *chrom = (*iter);
			int dag_count = 0;

			//cout<<"for each chromosone"<<flush<<endl;
			...
			//cout<<"before priority print in genetPath"<<flush<<endl;
			//for(unsigned i = 0; i < dag->allNodes.size(); i++)
			//{
			//	cout<<"in genet path, priority["<<i<<"]: "<<dag->allNodes.at(i)->priority<<endl;
			//}
			//cout<<"ima call my path scheduler"<<flush<<endl;
			int TS_time = ps.schedule(arch, dag);
			//ps.~PathScheduler();
			//cout<<"after delete the path scheduler"<<flush<<endl;
			if(firstFlag == true)
			{
				bestTS = TS_time;
				//cout<<"best TS genet path: "<<bestTS<<flush<<endl;
				firstFlag = false;
			}
			else
			{
		...
		//cout<<"init_pop2 size is: "<<init_pop->size()<<flush<<endl;
		...
		//cerr<<"CheckTime genet path is: "<<checkTime<<flush<<endl; To view the run time, uncomment this.

		if(checkTime > 1.8e+12)
		{
			goto timeOut;
		}
		//cout<<"checkTime iss: "<<checkTime<<flush<<endl;
		//timeoutTime.printElapsedTime();
...
timeOut:
	...
	//done using a "     " approach
	int TS_time = ps.schedule(arch, dag);
	cout<< "GenetPath Time: " << TS_time << " TS" << endl;
	delete tempor;
